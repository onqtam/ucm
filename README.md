...
- Something that builds fine as a normal build might not as a unity - for example static variables (or just in anonymous namespaces) with the same names in 2 different source files, type redefinitions or some macro shenanigans. ```using namespace xxx``` now becomes problematic even in normal source files.
